...
def hetero_graph_vectorize(include_node_feature=False, dataset="cresci-2015", server_id="209"):
    ...
    tweet_text = [text for text in tweet.description]
    ...
    for edge_type in edge_types:
        ...
        if edge_type == "post":
            ...
            graph["user", edge_type, "user"] = torch.LongTensor([src, dst])
            
    train_uid_with_label = user[user.split.str == "train"][["id", "split", "label"]]
    valid_uid_with_label = user[user.split.str == "val"][["id", "split", "label"]]
    ...
    if include_node_feature:
        ...
        return graph, uid_to_user_index, tid_to_tweet_index, user_text, tweet_text, train_uid_with_label, valid_uid_with_label, test_uid_with_label
